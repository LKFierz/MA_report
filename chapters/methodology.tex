...
Like Structure from Motion, the stereo depth program is a ros node which is given images and image poses from the xVIO state estimator. As the state estimator was in its final development stages during my thesis, simulation camera images and a ground truth camera pose were used instead as input for the stereo algorithm. Note that only one camera pose is given as the second one is derived in a straight forward manner, given the fixed baseline.
...
Once implemented the landing site detection instance could be supplied by the stereo depth node. The result can be seen in the following:
...
The autonomy handles the detected landing sites using a landing site manager which is constantly executed alongside the autonomys current state execution. This is shown in \cref{fig:autonomy}.
