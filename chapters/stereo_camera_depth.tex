...
\begin{itemize}
    \item $t_DC^W$: Transformation from Drone to Camera frame represented in the World frame
    \item $R_DC$: Active Rotation from the Drone to the Camera frame
    \item $r_DC^W$: Position vector from the drone to the camera represented in the World frame.
\end{itemize}
...
% The formulas for the coordinates are shown in \cref{}

\begin{align}
    y &= 
    z &= z \quad \text{the depth value is already stored in the depth image and can be retained directly.}\\
\end{align}
